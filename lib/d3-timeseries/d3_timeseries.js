// import * as d3 from "../d3/d3.min.js"; // Imported in index.html
...
export default function ()  {
  ...
  // Custom format
  xscale.setformat = date => {
    return (
      d3.timeSecond(date) < date ? d3.timeFormat('')(date) :
      d3.timeMinute(date) < date ? d3.timeFormat('%Mm%Ss')(date) :
      d3.timeHour(date) < date ? d3.timeFormat('%Mm')(date) :
      d3.timeFormat('')(date)
    )
  }
  ...
  '<td style="color:#333333;text-align:right">' + yscale.setformat(d.item[d.aes.y]) + '</td>' +
  '<td>'+ yscale.units + '</td></tr>';
    }).join('') + '</table>';
  ...
  var chart = function (elem)  {
    ...
    svg.append('g')
      ...
      .attr("x", d3.mean(yscale.range()))
      .attr("dy", ".71em")
      //.attr('y', -margin.left + 5)
      .attr('y', -width + margin.left)
      .attr('fill', "#000")
      .style("text-anchor", "middle")
      .text(yscale.label + " (" + yscale.units + ")");
  ...
  var scaleGetSet = function (scale) {
    return {
      ...
      units: function(_) {
        if (!arguments.length) return scale.units;
          scale.units = _;
        return chart;
      },
      domain: function (_) {
